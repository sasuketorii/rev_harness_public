...
shift
#
# codex-review-hook.sh - shell adapter for review-queue hook ingress (Codex)
#
# The caller-facing hook path is a shell adapter ingress for Codex
# PostToolUse compatibility. PostToolUse pre-filtering lives in this shell
# adapter so skipped files require no toolchain work (cargo-free on all paths).
