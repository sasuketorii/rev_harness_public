...
load_runtime_policy() {
  ...
  # Portable model override: ${SOURCE_POLICY_REL} current_model/minimum_allowed_model
  # are the harness maintainer's default baseline (see official_sources in that
  # file). Adopters running under their own account/plan may point this wrapper
  # at a different model id via REV_HARNESS_CODEX_MODEL without editing the
  # policy file. The fail-closed minimum-model gate above is NOT bypassed by
  # this override: the overridden id is re-ranked against the same
  # MINIMUM_ALLOWED_MODEL and the wrapper dies before invoking codex if it
  # ranks below the minimum, exactly as it would for the policy default.
  if [[ -n "${REV_HARNESS_CODEX_MODEL:-}" ]]; then
    local override_rank
    override_rank="$(model_rank "${REV_HARNESS_CODEX_MODEL}")" \
      || die "REV_HARNESS_CODEX_MODEL has unsupported model id format: ${REV_HARNESS_CODEX_MODEL}"
    [[ "${override_rank}" -ge "${minimum_rank}" ]] \
      || die "REV_HARNESS_CODEX_MODEL=${REV_HARNESS_CODEX_MODEL} is below minimum allowed model ${MINIMUM_ALLOWED_MODEL}"
    log_info "Model overridden via REV_HARNESS_CODEX_MODEL: ${FIXED_MODEL} -> ${REV_HARNESS_CODEX_MODEL}"
    FIXED_MODEL="${REV_HARNESS_CODEX_MODEL}"
  fi
...
Notes:
  ...
  - Caller overrides for profile, model, reasoning effort, sandbox, approval,
    web-search, and workspace-expansion controls are blocked via CLI flags.
  - The effective model id comes from ${SOURCE_POLICY_REL} (current_model) by
    default. Set REV_HARNESS_CODEX_MODEL=<model-id> to point this wrapper at a
    different model (e.g. your own account's available model) without editing
    the policy file. The fail-closed minimum-model gate still applies to the
    override: a model ranked below minimum_allowed_model is rejected before
    codex is invoked, the same as it would be for the policy default.
...
run_codex_attempt() {
  # Defense-in-depth: subscription-auth-guard.sh (run_subscription_auth_guard,
  # called earlier in main()) already fails closed if OPENAI_API_KEY /
  # CODEX_API_KEY are set in the wrapper's own environment. This additional
  # `env -u` strips those two vars from the actual codex child process itself,
  # so a future call path that reaches run_codex_attempt without having run
  # the guard first still cannot hand codex API-key auth. No behavioral
  # change for the normal path, where the guard has already died on any
  # non-empty value.
  if [[ -n "${TIMEOUT_BIN}" ]]; then
    "${TIMEOUT_BIN}" "${CODEX_WRAPPER_TIMEOUT_SECS}" env -u OPENAI_API_KEY -u CODEX_API_KEY "$@"
  else
