...
#
#   downstream の identity-dependent path (scripts/review-queue.sh の project_id
#   namespace 等) は
...
_rev_harness_emit_guard_notice() {
  ...
  case "$class" in
    ...
    invalid)
      ...
      printf '              Identity-dependent commands (e.g. scripts/review-queue.sh) will fail closed\n' >&2
      printf '              downstream until repaired. Repair with: bash scripts/rev-harness install --target <your-project>\n' >&2
      ;;
  esac
...
rev_harness_assert_canonical_root() {
  ...
  # downstream (scripts/review-queue.sh の project_id namespace 等) が project_id 必須なので、
